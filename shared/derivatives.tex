...
$\Gradient{f}[\Vector{u}} \in \Space{V}$ is the gradient of $f$ at $\Vector{u} \in \Space{V]$;
...
\Gradient{f}{\Vector{u}} = \sum_{j=0}^{m-1} \left( \db{j}{f}[\Vector{u}} \right) \Vector{e}_j^{\Space{V}]
\end{equation}
...
\Derivative{l_2^3}[\Vector{v}} = 3 \| \Vector{v}  \| \Vector{v]^\dagger \nonumber
\end{eqnarray}

Let $\Vector{f}(\Vector{v}) : \Space{V} \mapsto \Space{W}$.
By the chain rule:
$\Derivative{\| \Vector{f} \|^2}{\Vector{v}}  =  2 {\Vector{f}(\Vector{v})}^{\dagger} \Db{\Vector{f}}[\Vector{v}] $
and
$\Gradient{\| \Vector{f} \|^2}[\Vector{v}}  =  2 \Derivative{\Vector{f}}[\Vector{v}}^\dagger \circ \Vector{f](\Vector{v])$.
\begin{eqnarray}
...
$\Gradient{\Vector{f}_{kl}}[\Vector{L}_0}$ is an element of the domain of $\Vector{f]$,
